...
def test_prepare_lab2_inputs_smoke(tmp_path, run_cmd):
    ...
    run_cmd([
        ...
        "labs/lab2_asia/data/raw_templates/tiva_alt_example.csv",
        "--exgr-input",
        "labs/lab2_asia/data/raw_templates/tiva_exgr_example.csv",
        "--mappings",
    ...
    expected_cols = [
        "country", "year", "dva_value", "fnl_value", "exgr_value",
        "dva_share", "dva_ratio", "dva_growth", "dva_lag",
        "dva_share_growth", "dva_share_lag",
    ]
    ...
    assert df["dva_share"].notna().all(), "dva_share should be defined with EXGR denominator"
    assert ((df["dva_share"] > 0) & (df["dva_share"] <= 1)).all()
    ...
    assert summary["exgr_provided"] is True
    assert summary["dva_share_definition"] == "EXGR_DVA / EXGR"
...
def test_convergence_scaffold_smoke(tmp_path, run_cmd):
    ...
    assert -4.5 < summary["beta"] < -1.5, f"beta={summary['beta']} outside expected range (-4.5, -1.5)"
...
def test_prepare_then_convergence_integration(tmp_path, run_cmd):
    ...
    assert summary["method"] == "Beta_Convergence_OLS_HC1"
    assert summary["outcome_mode"] == "share"
    assert summary["year_fe"] is True
...
def test_convergence_share_mode_smoke(tmp_path, run_cmd):
    out_dir = tmp_path / "share_conv"
    out_dir.mkdir()
    ...
    summary = json.loads((out_dir / "model_summary.json").read_text(encoding="utf-8"))
    assert summary["outcome_mode"] == "share"
    assert summary["n_obs"] >= 100
    assert -4.5 < summary["beta"] < -1.5
    assert summary["convergence_detected"] is True
    assert summary["half_life_years"] > 0
