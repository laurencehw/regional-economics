...
| VIIRS Night Lights | Alternative proxy for economic activity in Africa | In progress | `labs/lab6_africa/data/raw/viirs/viirs_africa_YYYY.csv` (template via smoke-test); real rasters via NOAA/EOG VNL v2.1 | Fetch script built: `scripts/fetch_viirs_africa.py`. Smoke-test produces synthetic country-level panel (54 countries, `iso3, year, avg_radiance`). Adjacency edge list (220 pairs, `iso3, neighbor_iso3, shared_border_km`) also generated. Real pull requires pre-downloading VIIRS VNL v2.1 .tif from eogdata.mines.edu (registration required, ~200-500 MB/year) and passing `--local-tif`. rasterio 1.5.0 + geopandas 1.1.1 already installed. Natural Earth 110m boundaries used for zonal stats. |
