...
| WIOD / TiVA | MRIO and value-added decomposition in Asia | In progress | `data/raw/metadata/wiod_2016_pull_manifest_full_wiots_2026-02-22.json`, `data/raw/tiva/tiva_mainlv_asia_oecd_exgr_dva_2000_2023_2026-02-22.csv`, `data/processed/lab2/wiod_tiva_concordance_summary_2026-02-22.json` | WIOD 2016 pull expanded to full WIOTS package set (`WIOTS_in_R.zip`, `WIOTS_in_EXCEL.zip`) plus NIOTS/docs with size-validated manifest logging. OECD TiVA constrained extract remains 10 core Asian economies (`EXGR_DVA`, counterpart `OECD`, annual, USD; 230 rows, 2000-2022 observed). Lab 2 concordance templates are generated for countries and activity codebooks in `data/processed/lab2/`. |
...
| ACLED | Conflict event timing/intensity for MENA SCM | In progress | `docs/acled_access_workflow_2026-02-22.md`, `data/raw/metadata/acled_access_tracker_2026-02-22.json`, `labs/lab4_mena/data/acled_intake_checklist_2026-02-22.md` | Licensing/access track opened with role, scope, and storage controls documented. Pending: submit access request and record approval date before first real pull. |
...
## Near-Term Acquisition Queue (as of 2026-02-22)
1. Add at least one TiVA alternative measure extract (for example `EXGR_FVA`) and compare sensitivity against `EXGR_DVA`.
2. Submit and confirm ACLED licensing/access approval; log account and approval metadata in tracker JSON.
3. Stage UNHCR intake with a schema template aligned to Lab 4 panel keys.
