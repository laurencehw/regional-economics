...
| World Bank LPI (WDI `LP.LPI.OVRL.XQ`) | Broad Americas logistics-friction proxy for Lab 1 robustness | In progress | `data/raw/wdi/wdi_lpi_americas_long_2026-02-21.csv` (planned) | Fetch via `scripts/fetch_wdi_lpi_americas.py`; transform/blend via `scripts/derive_lab1_lpi_border_proxy.py` to reduce dependence on BTS-only coverage. |
