...
Look at the Korean peninsula from a satellite at night. The south is a blaze of light: Seoul, Busan, Incheon, the industrial corridors of the Yeongnam and Chungcheong regions. The north is almost entirely dark, lit only by the faint glow of Pyongyang. The 1945 occupation partition along the 38th parallel, followed by separate governments in 1948, created a natural experiment in which shared ethnolinguistic culture and prewar colonial history confront radically different postwar institutional regimes. That comparison is powerful, but it is not monocausal. South Korea's trajectory also depended on US security guarantees and aid, land reform that broke landlord power, Cold War market access, and a developmental state that directed credit and disciplined chaebol upgrading; North Korea's trajectory combined Soviet-style planning with militarized isolation and famine-inducing policy failures. Geography alone cannot explain the night-lights gap, and neither can a single institutional slogan. The useful claim is narrower: after 1945, institutional and geopolitical choices became the dominant drivers of divergence on an otherwise common peninsula.
...
{% hint style="info" %}
**Institutional Thickness.** Developed by Amin and Thrift (1994), *institutional thickness* refers to the density and quality of formal and informal institutions at the local level — property rights enforcement, contract reliability, regulatory transparency, business associations, and interpersonal trust. The concept captures the enormous subnational variation in institutional quality that exists even within countries with the same formal legal system. Regions with thick institutional environments convert market access into rising incomes; regions with thin environments do not.
{% endhint %}
...
{% hint style="info" %}
**Path Dependence.** An outcome is path-dependent when its current state depends on the sequence of prior events, not just on current conditions. In economics, path dependence arises when increasing returns, coordination failure, distributional conflict, or cultural legitimacy make it costly to switch from one institutional configuration to another — even when superior alternatives exist. The implication for regional policy is that history is not merely background; it is a structural determinant of which equilibria are reachable.
{% endhint %}
...
Both regions entered the postwar era as massive coal-and-steel complexes at the heart of their respective national economies. Both employed hundreds of thousands of workers in vertically integrated heavy industry. Both faced the same structural challenge from the 1960s onward: declining demand for coal, rising competition from lower-cost producers, and the secular shift of advanced economies toward services and knowledge-intensive manufacturing. Institutions are central to the divergence, but they are not the only difference: West Germany entered the transition as a high-income democracy inside the EC/EU market, with federal fiscal capacity and NATO security; the Donbas entered post-Soviet transition amid national state collapse, weak property rights, oligarchic privatization, and later war. The comparison therefore identifies institutional capacity under contrasting national environments, not a pure hold-institutions-fixed experiment.
...
The comparison underscores the chapter's central argument: path dependence is not deterministic. The same structural challenge (coal decline) produced radically different outcomes because the institutional environments differed in their capacity to manage distributional conflict, reconstruct cultural legitimacy, and overcome coordination failure. Alternative explanations still matter: EU market access, West German fiscal transfers, and the absence of interstate war cushioned the Ruhr, while Soviet mono-industrial planning, chaotic privatization, and later conflict punished the Donbas. The residual claim is that institutions mediate those shocks—they determine whether a region can convert resources, aid, or market access into a new growth path rather than into rent preservation.

{% hint style="info" %}
**Evidence and alternatives (Ruhr–Donbas).** This is a paired historical comparison, not a quasi-experiment with common national conditions. Security environment, EU market access, federal fiscal capacity, privatization design, and wartime destruction all differ. The chapter’s claim is mediation: institutions shape whether coal decline becomes managed *Strukturwandel* or extractive lock-in. It does not identify institutions as the sole sufficient cause of the divergence.
{% endhint %}
...
{% hint style="info" %}
**The Product Space.** Introduced by Hidalgo et al. (2007), the product space is a network in which nodes are products and edges connect products that are frequently co-exported by the same countries — a revealed-preference measure of shared productive capabilities. Products in the dense core (machinery, chemicals) offer many upgrade paths; products in the sparse periphery (raw commodities) offer few. A region's position in the product space determines which new industries it can realistically develop.
{% endhint %}
...
{% hint style="success" %}
**Smart Specialization (S3).** The EU's Smart Specialization Strategy requires each region to identify domains where it has existing capabilities *and* realistic potential for innovation — an "entrepreneurial discovery process" that operationalizes the related variety framework. Regions must build on what they already do well rather than imitating distant success stories. S3 is implemented at the NUTS-2 level, making it directly subject to the Modifiable Areal Unit Problem discussed in Chapter 3-A: whether NUTS-2 is the right geographic unit for identifying capability clusters is itself a substantive question.
{% endhint %}
...
{% hint style="info" %}
**Varieties of Capitalism (VoC).** Hall and Soskice (2001) distinguish Coordinated Market Economies (CMEs — Germany, Japan, Scandinavia), where firms coordinate through non-market institutions (employer associations, vocational training, patient capital), from Liberal Market Economies (LMEs — US, UK), where coordination occurs through competitive markets. The elements of each system are mutually reinforcing: extracting one component degrades the whole. The VoC framework predicts that CMEs generate spatially diffuse, incremental innovation, while LMEs generate spatially concentrated, radical innovation.
{% endhint %}
...
{% hint style="warning" %}
**Informality as Institutional Equilibrium.** De Soto (1989) reframed the informal economy not as a residual of backwardness but as a rational response to exclusionary formal institutions. When the cost of registering a business or obtaining legal title is prohibitively high, entrepreneurs build institutional frameworks outside the formal system. The consequence is "dead capital" — productive assets without legal recognition that cannot serve as collateral, limiting access to credit and constraining firm growth. Breaking the informality trap requires not just deregulation but building the enforcement infrastructure that makes formal contracts meaningful.
{% endhint %}
...
**Available data sources for subnational institutional measurement.** At the cross-country level, the World Governance Indicators (WGI) provide six dimensions (voice, stability, government effectiveness, regulatory quality, rule of law, corruption control) — but these are national aggregates that mask subnational variation. For subnational measurement, researchers can draw on: (1) the European Quality of Government Index (EQI), which surveys 200,000+ EU residents on perceived institutional quality at the NUTS-2 level; (2) Doing Business subnational indicators (discontinued in 2021 but available historically for ~190 cities); (3) Afrobarometer and Latinobarómetro regional modules on trust, corruption perception, and service delivery; (4) administrative data on court case resolution times, business registration days, and land title processing speeds, which proxy for institutional efficiency at the municipal level. For developing countries where survey infrastructure is sparse, composite indices built from nighttime luminosity (as a proxy for economic activity), mobile money penetration (as a proxy for financial inclusion), and election turnout data (as a proxy for civic engagement) can provide rough institutional thickness estimates at subnational scales — though all such proxies carry measurement error that should be acknowledged.
...
6. Chapter 2 treats institutional thickness as a regional stock that accumulates slowly. Choose one regional case later in the book (e.g., Monterrey, the Basque Country, Hsinchu, or the Northern Corridor) and identify which components of thickness — organizations, norms, inter-organizational ties — appear to have been built deliberately by policy versus inherited. What would falsify a purely institutional explanation of that region's performance?
