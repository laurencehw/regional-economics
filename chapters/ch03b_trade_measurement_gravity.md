...
The practical implication is that any gravity estimation that omits multilateral resistance will produce biased estimates. The standard solution is to include exporter and importer fixed effects, which absorb $$Y_i$$, $$E_j$$, $$P_j$$, and $$\Pi_i$$ entirely, leaving bilateral trade costs as the only source of identifying variation. Figure 3B.2 illustrates the distance-decay function estimated from bilateral trade data, showing how trade flows decline with distance at rates consistent with the canonical elasticity of $$-0.9$$ to $$-1.1$$.

![Figure 3B.2: Distance decay of bilateral trade flows — estimated gravity relationship](../figures/fig_ch03b_distance_decay.png)

*Source: Author's calculations using CEPII bilateral trade data.*
...
In practice, implementing PPML for gravity requires several decisions that affect results. Standard errors should be clustered at the country-pair (dyad) level when repeated bilateral flows are observed over time; some applications use exporter, importer, or multi-way clustering when the dependence structure warrants it (Cameron, Gelbach, and Miller 2011). Panel specifications should include exporter-time and importer-time fixed effects to absorb time-varying multilateral resistance. Convergence can be slow with high-dimensional fixed effects and many zeros; `ppmlhdfe` in Stata (Correia, Guimarães, and Zylkin 2020) and `fixest` in R are designed for this setting.

Coefficient interpretation depends on the regressor's form, not on PPML placing trade in levels. A coefficient on $$\ln d_{ij}$$ is an elasticity: $$\beta_1=-0.95$$ means a 1-percent increase in distance is associated with approximately 0.95 percent less expected trade. Doubling distance changes expected trade by $$2^{-0.95}-1\approx-48$$ percent. For a dummy variable with coefficient $$\beta$$, the exact percentage effect is $$100[\exp(\beta)-1]$$; for a level regressor changing by $$\Delta x$$, it is $$100[\exp(\beta\Delta x)-1]$$.

Figure 3B.3 compares PPML and OLS coefficient estimates for key gravity variables, illustrating the systematic differences that arise from the zero-trade problem and heteroskedasticity.

![Figure 3B.3: PPML vs. OLS gravity estimates — coefficient comparison](../figures/fig_ch03b_ppml_vs_ols.png)

*Source: Author's calculations. Illustrative comparison based on bilateral trade data.*
...
The practical consequence of the four-mode framework is that BOP-based services statistics capture much of Modes 1 and 2, only fragments of Mode 4, and almost none of Mode 3 sales. Mode 4 cannot be read directly from “workers' remittances”: remittances are secondary-income transfers, not payments for a service. Compensation of employees may include income earned by people present for less than a year, but residence rules, employment relationships, and the GATS requirement of temporary service supply do not align cleanly. Measuring Mode 4 therefore requires combining BOP service transactions with migration, visa, occupation, contract, and duration data.

Foreign-affiliate sales are often much larger than recorded cross-border services exports, but the two totals should not simply be added. FATS records the gross sales of resident affiliates, including local labor and intermediate inputs; BOP services records cross-border transactions between residents and nonresidents. They are different delivery measures with different consolidation and ownership rules, and summing them can double-count linked transactions. For gravity modeling, the practical implication is narrower: a BOP dependent variable describes recorded cross-border delivery, not the geography of commercial presence. Mode 3 should be analyzed separately with affiliate sales, employment, and ownership data where available.
...
Grossman and Rossi-Hansberg (2008) provided a theory in which production consists of tasks with heterogeneous offshoring costs. The two-dimensional classification below is a pedagogical synthesis of that model with the broader task-content and offshorability literature (Autor, Levy, and Murnane 2003; Blinder 2009); it is not a matrix derived in Grossman and Rossi-Hansberg:
...
This phenomenon — the growing service content embedded in manufactured exports — is called **servicification**. Koopman, Wang, and Wei (2014) developed a detailed decomposition of gross exports into domestic value added, foreign value added, returned domestic value, and double-counted components. OECD TiVA uses inter-country input-output tables to trace value by country and sector of origin. At the broadest level, domestic and foreign value-added shares describe where the value embodied in exports originated. **DVX is not a third mutually exclusive component to add beside DVA and FVA**: it is domestic value added embodied in a partner's subsequent exports and is therefore a use or destination component of domestic value added. Analysts must use the database's exact indicator definitions rather than summing nested measures. The sector-of-origin results are especially useful for measuring servicification:

- In Germany, services account for 40 percent of the domestic value-added in manufactured exports — up from 30 percent in 2000.
...
Three related TiVA indicators answer different questions:

1. **Domestic value added in gross exports (DVA):** Value created by domestic factors and embodied in the export, whether absorbed by the direct importer or passed onward through a production chain.

2. **Foreign value added in gross exports (FVA):** Imported value added embodied in the reporting country's exports.

3. **Domestic value added embodied in third-country exports (DVX):** The subset of domestic value added that a direct importing partner uses in its own exports. DVX measures forward GVC participation and must not be added to total DVA as though it were disjoint.
...
3. **STRI identification exercise.** A sector-average importer STRI cannot be added to the equation above while retaining importer fixed effects: $$\text{STRI}_j$$ is perfectly collinear with $$\gamma_j$$. Use sector-disaggregated flows and estimate:

$$
\text{Trade}_{ijs}^{services} = \exp(\alpha_{is} + \gamma_j + \delta_s + \beta_1 \ln d_{ij} + \beta_2 \text{contig}_{ij} + \beta_3 \text{lang}_{ij} + \beta_4 \text{colony}_{ij} + \beta_5 \text{STRI}_{js}) \cdot \eta_{ijs},
$$

where $$s$$ indexes service sectors. The coefficient is identified from differences in sectoral restrictiveness within importers and across importers, conditional on exporter-sector, importer, and sector effects. It remains vulnerable to omitted importer-sector multilateral resistance and should not be given an automatic causal interpretation. As a diagnostic, replace $$\gamma_j+\delta_s$$ with importer-sector fixed effects: the unilateral STRI main effect must then drop because it is absorbed. A bilateral interaction—such as $$\text{STRI}_{js}\times\text{MRA}_{ij,s}$$—can remain identified, but it estimates how a bilateral recognition arrangement moderates restrictiveness, not the level effect of STRI.

**Illustrative coefficient pattern—not an empirical result.**
...
If the variables are scaled appropriately, students can translate the STRI coefficient into the effect of a one-standard-deviation change using $$100[\exp(\hat\beta_5\Delta\text{STRI})-1]$$. A reduction in the absolute distance coefficient after adding STRI is consistent with omitted regulatory heterogeneity, but it does not prove that a fixed fraction of the distance effect was “actually” regulatory: coefficient changes can also reflect sample composition, functional form, measurement error, and correlated omitted variables.
...
6. A TiVA table shows rising domestic value-added in gross exports for an ASEAN economy's electronics sector. List three interpretations consistent with that increase (capability upgrading; changing product mix; changing import prices or exchange rates). What additional indicators — beyond a single DVA share — would you require before calling the pattern "servicification" or "convergence"?
