...
The commercial layer is dominated by ESRI (ArcGIS), Google (Earth Engine), Microsoft (Planetary Computer), and Meta (High Resolution Settlement Layer). These platforms generate massive positive externalities — Casaburi and Troiano (2016) document that Italian tax authorities used satellite imagery to detect roughly two million unregistered buildings, recovering an estimated EUR 472 million in evaded taxes — but commercial dominance creates lock-in risks for researchers who build pipelines around proprietary APIs.

Government-provided data functions as a competitive equalizer. Nagaraj (2021) shows that when NASA made Landsat imagery freely available in 2008, gold discovery rates nearly doubled and the market share of new entrants in mineral exploration rose from roughly 10 to 25 percent. The major providers — USGS, ESA Copernicus, NASA SEDAC, and NOAA — form the observational backbone on which most academic spatial analysis rests. An open-source ecosystem (OpenStreetMap, QGIS, geopandas, PySAL, R's sf/spdep stack) enables publication-quality spatial analysis without software costs, aligning naturally with the reproducibility principles discussed below.

Geospatial data does not only measure economic activity; it shapes it. Yang, Ai, and Arkolakis (2025) train machine learning models on pollutants, nighttime lights, and surface temperature to predict steel plant locations, achieving output prediction $$R^2$$ exceeding 0.8 — extending Henderson-Storeygard-Weil (2012) to plant-level inference. Nagaraj, Stern, and Scott Morton (2022) find that placing small businesses on online map listings increases revenue by approximately 5 percent. The infrastructure of spatial measurement is itself an institution, shaping incentives in precisely the sense that Chapter 2 described.
