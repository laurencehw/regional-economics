...
Moran's $$I$$ is the spatial analog of the Durbin-Watson statistic in time series: a first-pass check that tells you whether spatial dependence is worth modeling. A large, significant $$I$$ on OLS residuals says "something spatial is going on" — it does not say whether the dependence is in the dependent variable (SAR), the errors (SEM), or both (SDM). The LM tests in Section 3A.2 provide that discrimination. Figure 3A.3 illustrates a Moran scatter plot constructed from simulated regional data, showing the characteristic positive slope that indicates spatial autocorrelation.

![Figure 3A.3: Moran scatter plot — observed spatial autocorrelation in regional data](../figures/fig_ch03a_moran_scatter.png)
...
This sequence provides a disciplined approach to model selection that avoids data-mining across spatial specifications. The Data in Depth box below demonstrates it with NUTS-2 data. Figure 3A.4 displays SAR coefficient estimates across alternative specifications, illustrating how the spatial lag parameter and covariate coefficients shift when moving from OLS to SAR to SDM.

![Figure 3A.4: SAR coefficient estimates across model specifications](../figures/fig_ch03a_sar_coefficients.png)
...
1. **Predetermined networks.** Use $$W$$ from a prior period, on the assumption that lagged trade patterns are plausibly exogenous to current outcome innovations. Lab 1 uses contemporaneous trade data, which is a vulnerability; a robustness check using five-year lagged trade would strengthen identification. The cost is that lagged networks may misrepresent current linkages — a bias-variance tradeoff between endogeneity and measurement error.

2. **Gravity-predicted networks.** Construct $$W$$ from the predicted values of a gravity model (distance, common language, colonial ties), purging contemporaneous endogeneity. Frankel and Romer (1999) pioneered this approach. The exclusion restriction is that geographic and historical variables affect outcomes only through the trade channel — strong but testable.

3. **Geographic networks.** Use contiguity or inverse distance, accepting reduced economic specificity in exchange for exogeneity. Geography does not change in response to outcomes. The cost is that physical proximity imperfectly proxies economic integration — North and South Korea are adjacent but disconnected.

For descriptive analysis, geographic $$W$$ is sufficient. For policy evaluation, economic $$W$$ with careful attention to endogeneity is necessary. The regional labs use both approaches and compare results.
...
The WTO's BOP-based services trade statistics, the OECD STRI (22 sectors, 50+ countries), the World Bank's parallel STRI for developing economies (Borchert, Gootiiz & Mattoo 2014), and the ECIPE Digital Trade Estimates database collectively map services trade flows and regulatory barriers. Lab 7 uses these to estimate tariff-equivalent costs of regulation. BOP data systematically undercount services trade by missing Mode 3 (commercial presence); total services trade may be 50-70 percent larger once Mode 3 is included (see Chapter 3-B, Section 3B.2).
...
These datasets are products of a layered infrastructure of satellites, government agencies, open-source communities, and commercial providers. Nagaraj and Stern (2020) frame maps as economic goods that bundle data and design, arguing that cartographic representation is never neutral. Understanding who collects, curates, and controls spatial data is essential context for empirical analysis.

The commercial layer is dominated by ESRI (ArcGIS), Google (Earth Engine), Microsoft (Planetary Computer), and Meta (High Resolution Settlement Layer). These platforms generate massive positive externalities — Casaburi and Troiano (2016) document that Italian tax authorities used satellite imagery to detect two million unregistered buildings, recovering an estimated EUR 472 million in evaded taxes — but commercial dominance creates lock-in risks for researchers who build pipelines around proprietary APIs.

Government-provided data functions as a competitive equalizer. Nagaraj (2021) shows that when NASA made Landsat imagery freely available in 2008, gold discovery rates nearly doubled and the market share of new entrants in mineral exploration rose from 10 to 25 percent. The major providers — USGS, ESA Copernicus, NASA SEDAC, and NOAA — form the observational backbone on which most academic spatial analysis rests. An open-source ecosystem (OpenStreetMap, QGIS, geopandas, PySAL, R's sf/spdep stack) enables publication-quality spatial analysis without software costs, aligning naturally with the reproducibility principles discussed below.

Geospatial data does not only measure economic activity; it shapes it. Yang, Ai, and Arkolakis (2025) train machine learning models on pollutants, nighttime lights, and surface temperature to predict steel plant locations, achieving output prediction $$R^2$$ exceeding 0.8 — extending Henderson-Storeygard-Weil (2012) to plant-level inference. Luca, Nagaraj, and Subramani (2022) find that placing small businesses on online map listings increases revenue by about 5 percent. The infrastructure of spatial measurement is itself an institution, shaping incentives in precisely the sense that Chapter 2 described.
...
Every regression in this chapter depends on a prior decision: where do regions begin and end? In Europe, this question is answered by the NUTS classification (Nomenclature of Territorial Units for Statistics), maintained by Eurostat and updated every three years. The current version, NUTS 2024, defines 104 NUTS-1 regions, 244 NUTS-2 regions, and 1,166 NUTS-3 regions across the EU-27.
...
## Conclusion: Tools in Service of Questions
...
6. A researcher reports that after residualizing GDP growth on governance quality, Moran's $$I$$ falls from 0.35 to 0.08 and becomes insignificant. Does this prove that spatial clustering was "only" governance? What alternative data-generating processes could produce the same pattern, and what additional diagnostics would you require before interpreting the residual Moran's $$I$$ as evidence against geographic spillovers?
