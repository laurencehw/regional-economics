# Chapter 3-A: Spatial Econometrics — Methods for Regional Analysis

---

## Introduction: The Map Is Not the Residual

The canonical growth regression — income growth regressed on initial income to test for convergence — has been a staple of regional economics since Barro and Sala-i-Martin (1992). Applied to European NUTS-2 regions with OLS, these regressions consistently find convergence rates broadly consistent with neoclassical theory: poorer regions catching up at roughly 2 percent per year. But when Ertur and Koch (2007) reestimated the Mankiw-Romer-Weil (1992) growth model using a spatial autoregressive specification — one that explicitly allowed the growth rate of each region to depend on the growth rates of its neighbors — the convergence coefficient changed by nearly a third, and the estimated speed of convergence slowed substantially. The "missing" convergence had been absorbed into a spatial spillover term that the original OLS regression had forced into the error term.
...
6. **Combine space and time.** Section 3A.6 extends the cross-sectional spatial models to panel data, introducing spatial panel specifications that allow researchers to control for unobserved regional heterogeneity while modeling spatial dependence — the workhorse approach for Labs 1, 2, 3, and 4.

7. **Navigate the data landscape.** Section 3A.7 surveys the key datasets that the Applied Labs in this book draw on, and discusses reproducibility principles for spatial empirical work.
...
The datasets enumerated above do not exist in a vacuum. They are products of a geospatial data ecosystem — a layered infrastructure of satellites, software platforms, government agencies, open-source communities, and commercial providers that collectively determine what spatial phenomena can be measured, by whom, and at what cost. Nagaraj and Stern (2020) frame maps as economic goods that bundle data and design, arguing that the act of cartographic representation is never neutral: every map reflects the mapmaker's incentives, whether commercial, governmental, or scientific. The global geospatial industry generates an estimated \$340–400 billion in annual value, yet much of the analytical leverage in regional economics comes from data that is free at the point of use. Understanding the institutional architecture of this ecosystem — who collects, who curates, who profits, and who is excluded — is essential context for any empirical spatial analysis.

**Private platforms and commercial infrastructure.** The commercial layer of the geospatial ecosystem is dominated by a handful of firms whose products shape how researchers, planners, and policymakers see space. ESRI's ArcGIS platform remains the industry-standard GIS software, and its Living Atlas aggregates thousands of curated spatial datasets. Google Maps serves over one billion monthly users and operates on an advertising model that makes spatial data a loss leader for location-based commerce; Google Earth Engine extends this infrastructure to researchers, offering petabyte-scale satellite imagery processing through a cloud API. Microsoft's Planetary Computer provides similar research-grade access to environmental and land-use datasets. Meta's High Resolution Settlement Layer uses machine learning on satellite imagery to estimate population at 30-meter resolution — critical for regions where census data is decades old. Amazon's Registry of Open Data and CARTO's spatial analytics platform round out an ecosystem characterized by massive positive externalities. The social returns to spatial data frequently dwarf the private returns: Casaburi and Troiano (2016) document that Italian tax authorities used high-resolution satellite imagery to detect roughly two million unregistered "ghost buildings," recovering an estimated €472 million in previously evaded property taxes. But commercial dominance also creates lock-in risks: researchers who build pipelines around proprietary APIs face switching costs that can subtly constrain the questions they ask.

**Public data as competitive equalizer.** Government-provided geospatial data functions as a competitive equalizer — reducing informational barriers to entry in both economic activity and research. Nagaraj (2021) provides the cleanest evidence: when NASA made Landsat satellite imagery freely available in 2008, gold discovery rates in the United States nearly doubled, and the market share of new entrants in mineral exploration rose from roughly 10 to 25 percent. Information that had previously been the proprietary advantage of large mining firms became a public good. The major government providers — USGS (Landsat, elevation models), ESA's Copernicus program (Sentinel-1 radar, Sentinel-2 optical), NASA SEDAC (gridded population, environmental exposure), and NOAA (weather, ocean, and the VIIRS nighttime lights discussed above) — collectively form the observational backbone on which most academic spatial analysis rests. Their continued funding is not a technocratic detail; it is a precondition for the kind of evidence-based regional policy analysis this book advocates.

**The open-source ecosystem.** Complementing both private platforms and public data providers, an open-source software ecosystem has matured to the point where a researcher with no software budget can conduct publication-quality spatial analysis. OpenStreetMap — sometimes described as "the Wikipedia of maps" — provides volunteer-contributed, openly licensed geographic data covering road networks, buildings, land use, and points of interest worldwide. Boeing (2025) developed OSMnx, a widely used Python library for downloading and analyzing street network topology from OpenStreetMap data. The GeoAI project (opengeos) integrates machine learning with satellite imagery analysis. QGIS offers a full-featured open-source alternative to commercial GIS software. Together with the Python spatial stack (geopandas, rasterio, PySAL) and R's parallel ecosystem (sf, spdep, spatialreg), these tools constitute an alternative to proprietary lock-in — one that aligns naturally with the reproducibility principles discussed in the next subsection.

**Frontier sensing technologies.** Optical satellite imagery, for all its power, has limitations: clouds obscure the surface, nighttime restricts visible-spectrum observation, and two-dimensional images cannot capture vertical structure. Frontier sensing technologies address each constraint. LIDAR (Light Detection and Ranging) produces three-dimensional point clouds that map terrain, building heights, and vegetation canopy with centimeter-level precision — applications range from flood risk modeling to urban morphology classification. Synthetic Aperture Radar (SAR) penetrates cloud cover and operates day or night, making it invaluable for tropical regions where optical imagery is frequently unusable. Hyperspectral sensors capture hundreds of narrow wavelength bands, enabling identification of crop types, mineral deposits, and pollution plumes. The USGS 3D Elevation Program (3DEP) aims to provide nationwide LIDAR coverage for the United States, and ESA's Sentinel-1 constellation provides free global SAR data.

**Measurement and reflexivity.** These data sources are not merely inputs to analysis — they are increasingly the analysis itself. Yang, Ai, and Arkolakis (2025) train machine learning models on ambient air pollutants, nighttime light intensity, and surface temperature to predict the locations of individual steel plants across countries, achieving detection rates of 40–70 percent and output prediction with $$R^2$$ exceeding 0.8. This extends the Henderson-Storeygard-Weil (2012) framework from aggregate GDP proxying to industry-level, plant-level inference — a methodological frontier that Lab 6 and Chapter 16 explore further. But geospatial data does not only measure economic activity; it shapes it. Nagaraj, Stern, and Scott Morton (2022) find that placing small businesses on online map listings increases their revenue by approximately 5 percent — the map creates the economic geography it purports to describe. Larcom, Rauch, and Willems (2017) show that the London Tube map's topological distortions — distances are not drawn to scale — caused commuters to take systematically suboptimal routes, and a temporary line closure that forced experimentation led to lasting behavioral changes. The infrastructure of spatial measurement is itself an institution, shaping incentives and outcomes in precisely the sense that Chapter 2 described. The map is not the territory — but increasingly, the territory reorganizes itself around the map.
...
- **Lab 3 (South Asia):** Spatial concentration analysis of India's IT-BPO exports using state-level RBI and NASSCOM data, mapping the extreme geographic clustering of services exports and testing whether Tier-2 cities are reducing spatial inequality in the sector.
...
Every regression in this chapter depends on a prior decision: where do regions begin and end? In Europe, this question is answered by the NUTS classification (Nomenclature of Territorial Units for Statistics), maintained by Eurostat and updated approximately every three years. The current version, NUTS 2024, defines 104 NUTS-1 regions, 244 NUTS-2 regions, and 1,166 NUTS-3 regions across the EU-27.
